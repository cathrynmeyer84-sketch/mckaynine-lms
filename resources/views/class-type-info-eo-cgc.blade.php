...
{{-- 4. ABOUT (full-width block if present, no left/right col) --}}
@if($classType->about)
<section style="background:{{ $p['right_col'] }};padding:1.25rem 1.5rem;border-top:1px solid rgba(0,0,0,0.06);">
    <p class="section-heading">About This Class</p>
    <p class="body-text" style="white-space:pre-line;">{{ $classType->about }}</p>
</section>
@endif
...
<section class="two-col" id="class-details">
    ...
    <div class="left-col">
        ...
        <div>
            <p class="section-heading">Requirements</p>
    ...
    <div class="right-col">
        ...
        <div>
            <p class="section-heading">
                @if($selectedClass) This Class @else When &amp; Where… @endif
            </p>

            @if($selectedClass)
                @php
                    $ctaTypeInline = $classType->cta_type ?? 'enquire';
                    $classIdInline = $selectedClass->id;
                    $groupedDates = $selectedClass->scheduledDates->groupBy(fn($d) => $d->date->format('F Y'));
                @endphp
                @if($classType->general_schedule)
                <p class="body-text" style="margin-bottom:0.75rem;color:#6b7280;">{{ $classType->general_schedule }}</p>
                @endif
                <div style="background:white;border-radius:0.5rem;padding:0.75rem 1rem;display:flex;flex-direction:column;gap:0.4rem;margin-bottom:0.75rem;">
                    ...
                    <div style="margin-top:0.5rem;display:flex;flex-direction:column;gap:0.6rem;">
                        ...
                        <div>
                            <p class="body-text" style="font-weight:700;margin-bottom:0.2rem;">{{ $month }}</p>
                            <p class="body-text" style="color:#6b7280;">{{ $dates->map(fn($d) => $d->date->format('D d'))->join(' · ') }}</p>
                        </div>
                        @endforeach
                    </div>
                    @endif
                ...
                @php
                    $inlineEnrolUrl = route('enrol.choose-dog') . '?class_id=' . $classIdInline;
                @endphp
                @if($ctaTypeInline === 'enquire')
                <a href="{{ route('class-info.enquire.form', $classType->slug) }}?class_id={{ $classIdInline }}"
                    style="display:inline-block;background:{{ $p['btn_bg'] }};color:{{ $p['btn_text'] }};font-weight:800;font-size:0.9rem;padding:0.5rem 1.5rem;border-radius:2rem;text-decoration:none;margin-bottom:0.75rem;">
                    Enquire Now →
                </a>
                @else
                ...
                <a href="{{ route('class-info.show', $classType->slug) }}"
                    style="font-size:0.78rem;color:{{ $p['accent'] }};text-decoration:underline;display:block;">← View all available dates</a>
            ...
            @elseif($classType->individual_class_pages)
                ...
                <div style="display:flex;flex-direction:column;gap:0.5rem;">
                    ...
                    <div x-data="{ open: false }" style="background:white;border:1.5px solid {{ $p['left_col'] }};border-radius:0.5rem;overflow:hidden;">
                        ...
                        @php
                            $listEnrolUrl = route('enrol.choose-dog') . '?class_id=' . $c->id;
                        @endphp
            ...
            @else
                ...
                <p class="body-text" style="margin-bottom:0.75rem;">{{ $classType->general_schedule }}</p>
                @endif
                <p class="body-text" style="color:#6b7280;font-style:italic;">Enquire below to find out about upcoming dates.</p>
            @endif
        </div>
        ...
        <div>
            <p class="section-heading">What To Bring</p>
            <ul style="list-style:disc;padding-left:1.1rem;margin:0;display:flex;flex-direction:column;gap:0.2rem;">
...
$footerEnrolUrl = route('enrol.choose-dog') . ($classId ? '?class_id='.$classId : '');
@endphp
...
<footer class="footer">
    ...
    <div class="footer-bubble">
        Ready to take<br>the next step?<br>Join us today!
    </div>
