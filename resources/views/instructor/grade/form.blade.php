    ...
    <div class="space-y-3 mb-4">
        ...
        <div class="card">
            ...
            <div class="space-y-2">
                ...
                <button type="button"
                    @click="selectRating('{{ $ex->id }}', {{ (float)$scale->marks_deducted }}, {{ $scale->is_automatic_fail ? 'true' : 'false' }}, '{{ addslashes($scale->label) }}')"
                    :class="exs['{{ $ex->id }}'].selectedLabel === '{{ addslashes($scale->label) }}'
                        ? '{{ $scale->is_automatic_fail ? 'bg-red-600 border-red-600 text-white' : 'bg-navy border-navy text-white' }}'
                        : 'bg-white border-gray-200 text-gray-700 hover:border-navy'"
                    class="w-full border-2 rounded-xl py-2.5 px-4 text-left transition-all">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <p class="font-semibold text-sm">{{ $scale->label }}</p>
                            @if($scale->description)
                            <p class="text-xs mt-0.5 opacity-70">{{ $scale->description }}</p>
                            @endif
                        </div>
                        <div class="shrink-0 text-right">
                            @if($scale->is_automatic_fail)
                            <span class="text-xs font-medium opacity-80">⚠ Auto-fail</span>
                            @elseif($scale->marks_deducted > 0)
                            <span class="text-xs opacity-70">–{{ $scale->marks_deducted }} pts</span>
                            @else
                            <span class="text-xs opacity-70">no deduction</span>
                            @endif
                        </div>
                    </div>
                </button>
